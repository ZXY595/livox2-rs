...
pub struct PointDataStream {
    socket: UdpSocket,
    buffer: Vec<u8>,
}

impl PointDataStream {
    ///  1024 * 1024 * 200 in orginal livox sdk
    const INIT_BUFFER_SIZE: usize = 1380;
...
impl super::LidarStream for PointDataStream {
    type Packet<'a> = EthernetPacketRef<'a>;
    async fn from_config(config: &super::LidarConfig) -> Result<Self, io::Error> {
        Self::new(
            (config.local_ip, config.local_point_data_port),
            (config.lidar_ip, config.lidar_point_data_port),
        )
        .await
    }
